...
def copy_from_parcel_codes_to_postgres(conn, path_to_parcel_codes, table_name, sep=",", skip_header=True):
    cur = conn.cursor()
    book = xlrd.open_workbook(filename=path_to_parcel_codes, encoding_override="latin_1")
...
def copy_division_to_postgres(conn, path_to_parcel_codes, table_name, sep=",", skip_header=True):
    cur = conn.cursor()
    book = xlrd.open_workbook(filename=path_to_parcel_codes, encoding_override="latin_1")
    ...
    if os.path.exists(shapefile_path):
        ...
        with fiona.open(shapefile_path) as shapefile:
            for feat in shapefile:
                ...
                query += (
                    "VALUES (ST_SetSRID(ST_GeomFromText(%s),31370), "
                    + ",".join(["%s" for column_name in columns])
                    + ")"
                )
...
def build_full_genealogy(array):
    ...
    new_array = pandas.DataFrame(data=new_historic, columns=columns)
    return new_array
...
def main():
    ...
    conn = psycopg2.connect(
        "host=%s dbname=%s user=%s password=%s port=%s" % (pg_host, database_name, user_name, user_password, port)
    )
    check_postgis()
    ...
    copy_from_array_to_postgres(conn, new_historic_array, "parcels_historic", skip_header=True)
    ...
    copy_from_array_to_postgres(conn, reduced_historic_array, "reduced_parcels_historic", sep=";", skip_header=True)
    genealogy_array = build_full_genealogy(reduced_historic_array)
    print("* Importing genealogy of each capakey")
    copy_from_array_to_postgres(conn, genealogy_array, "complete_parcels_genealogy", skip_header=True)
    ...
    copy_from_array_to_postgres(conn, capakey_genealogy_array, "parcels_genealogy", skip_header=True)
    copy_from_csv_to_postgres_copy(conn, path_to_owner, "owners_imp", sep=";", skip_header=True)
    copy_from_csv_to_postgres_copy(conn, path_to_parcel, "parcels_imp", sep=";", skip_header=True)
    copy_from_parcel_codes_to_postgres(conn, path_to_parcel_codes, "global_Natures", sep=";", skip_header=True)
    print("* Filling tables")
    filling_tables(conn, cadastre_date)

    copy_division_to_postgres(conn, path_to_parcel_codes, "global_Natures", sep=";", skip_header=True)
    clean_unused_division(conn)

    load_shapefile(conn, "capa", path_to_capa, ["CaPaKey", "CaSeKey"])
