...
dO = pd.read_csv(
    path_to_matrice + "/Owner.csv",
    sep=";",
    header=0,
    encoding="latin_1",
    na_filter=False,
    dtype={"articleOrder": str},
)
dP = pd.read_csv(
    path_to_matrice + "/Parcel.csv",
    sep=";",
    header=0,
    encoding="latin_1",
    na_filter=False,
...
divCadF = pd.read_excel(
    fileparcels[0], sheet_name="divCad ", skiprows=1, names=["divCad", "divName_o"]
).assign(divName="")
divCadF["divName"] = divCadF["divName_o"].apply(lambda x: convDivName(x))

# Récupération du nom de la nature de la parcelle
natureF = pd.read_excel(
    fileparcels[0],
    sheet_name="Nature",
    skiprows=1,
    names=["nature", "nature_name", "nature_name_nl"],
)

da = pd.DataFrame(data=dP.divCad.unique(), columns=["divCad"])
da["divName"] = da["divCad"].apply(
    lambda x: divCadF[divCadF.divCad == x].divName.values[0]
)
da["dan1"] = da["divName"]
...
pe.loc[:, ["pe"]] = pe.name.str.cat(
    pe.firstname.astype(str), sep=", "
)  # pe = nom, prénom
pe.loc[:, ["pe"]] = (
    pe.pe.astype(str)
    + "(cj."
    + pe.name_partner.astype(str)
    + ", "
    + pe.firstName_partner.astype(str)
    + ")"
)  # pe = nom, prénom
pe.loc[:, "pe"] = pe.pe.str.replace("\\(cj., \\)", "")
pe.loc[:, "pe"] = pe.pe.str.replace(
    "[,] $", ""
)  # retirer la virgule à la fin du nom isolé


dO["articleNumber"] = pd.to_numeric(
    dO["articleNumber"], errors="coerce"
)  # crée des NaN si rien dans le champ
...
def getNatureNameFromIndex(natureIndex):
    ...
    if not natureItem.empty:
        ...
        print(
            "Erreur lors de la récupération de la valeur de nature pour le code %s . Il faut vérifier export_parcels.xlsx page nature"
            % natureIndex
        )
...
prc.loc[:, "prc"] = prc.prc.str.replace(
    " $", ""
)  # retirer l'espace à la fin du nom isolé

prc["street_situation"] = (
    prc["street_situation"].astype(str) + " " + prc["number"].astype(str)
)
...
map = map.merge(
    prc.drop_duplicates(subset=["capakey"]),
    how="left",
    left_on="CAPAKEY",
    right_on="capakey",
)
map = map.merge(
    pe.drop_duplicates(subset=["propertySituationIdf"]),
    how="left",
    left_on="propertySituationIdf",
    right_on="propertySituationIdf",
)
...
pe.to_csv(
    path_to_data + "/o_pe.csv",
    sep="|",
    columns=["pe", "pos", "adr1", "adr2", "daa", "lt"],
)
...
prc.rename(
    columns={
        ...
        "cadastralIncome": "ri1",
        "street_code": "rscod",
        "constructionYear": "acj",
    },
    inplace=True,
)
prc.to_csv(
    path_to_data + "/o_prc.csv",
    sep="|",
    columns=[
        "capakey",
        "daa",
...
map.rename(
    columns={"divCad": "da", "primaryNumber": "radical", "bisNumber": "bis"},
    inplace=True,
)
...
capa = capa.merge(
    map.loc[
        :, ["CAPAKEY", "da", "section", "radical", "exposant", "bis"]
    ].drop_duplicates(subset=["CAPAKEY"]),
    how="left",
    left_on="CAPAKEY",
    right_on="CAPAKEY",
)
